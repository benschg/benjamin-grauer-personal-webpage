...
import { documents } from "@/components/working-life/content";
  ...
  return (
    ...
    >
      <CardContent sx={{ textAlign: "center", py: 3 }}>
        <Box sx={{ color: "#89665d", mb: 2 }}>{icon}</Box>
        <Typography
          variant="h6"
          sx={{
            fontFamily: "Orbitron",
...
const CVAttachmentCards = () => {
  ...
  return (
    ...
    >
      ...
      >
        {documents
          .filter((doc) => doc.title === "References" || doc.title === "Certificates")
          .map((doc) => (
            <AttachmentCard
              key={doc.title}
              title={doc.title}
              ...
              icon={
                doc.title === "References" ? (
                  <ContactPage sx={{ fontSize: 48 }} />
                ) : (
                  <EmojiEvents sx={{ fontSize: 48 }} />
                )
              }
              pdfPath={doc.downloadUrl}
              downloadName={doc.downloadAs}
            />
          ))}
      </Box>
    </Box>
