...
function generateLetterHtml(letter: MotivationLetter, candidateName: string, theme: 'dark' | 'light'): string {
  ...
  const bodyParagraphs = letter.body
    ...
    .map(p => p.trim())
    .filter(p => p.length > 0)
    .map(p => `<p style="margin: 0 0 1em 0; line-height: 1.6;">${p}</p>`)
    .join('');
...
<body>
  <div class="letter-container">
    ...
    <div class="body-section">
      <p style="margin: 0 0 1em 0; line-height: 1.6;">${letter.opening}</p>
      ${bodyParagraphs}
      <p style="margin: 0 0 1em 0; line-height: 1.6;">${letter.closing}</p>
    </div>

    <div class="signoff">${letter.signoff}</div>
